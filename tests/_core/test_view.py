...
def test_view_init(dummy_saliency_grid):
    scoped = SaliencyView(dummy_saliency_grid, batch_idx=1, image_idx=0)
    ...
    scoped_2 = dummy_saliency_grid.view(batch_idx=1, image_idx=0)
...
def test_view_input_ids(dummy_saliency_grid):
    ...
    scoped_2 = dummy_saliency_grid.view(batch_idx=1, image_idx=0, input_ids=input_ids[1])
    assert torch.equal(scoped_2.input_ids, scoped.input_ids)
    assert torch.equal(scoped_2.gen_tokens, scoped.gen_tokens)


def test_view_processor_tokenizer(dummy_saliency_grid, dummy_tokenizer, dummy_processor):
    input_ids = dummy_input_ids()
    scoped = SaliencyView(
        dummy_saliency_grid,
        batch_idx=1,
    ...
    assert scoped["token_20"] == scoped[0]  # Regex selection

    scoped_2 = dummy_saliency_grid.view(
...
def test_view_selector(dummy_saliency_grid):
    input_ids = dummy_input_ids()
    scoped = SaliencyView(dummy_saliency_grid, batch_idx=1, image_idx=0, input_ids=input_ids)
...
def test_iter_selector(dummy_saliency_grid):
    input_ids = dummy_input_ids()
    scoped = SaliencyView(dummy_saliency_grid, batch_idx=1, image_idx=0, input_ids=input_ids)

    assert len(scoped) == scoped.num_tokens

    for i, map in enumerate(scoped):
...
def test_view_plot(dummy_saliency_grid):
    image = Image.new("RGB", (16, 16), color="white")
    scoped = SaliencyView(dummy_saliency_grid, batch_idx=1, image_idx=0, image=image)
...
def test_view_visualize(dummy_saliency_grid, dummy_tokenizer):
    scoped = SaliencyView(dummy_saliency_grid, batch_idx=1, image_idx=0)
