...
# ----- SaliencyOutput tests -----


def test_saliency_output_forwards_dataclass_fields(layout, tensor, mask):
    gen_mask = mask(B=1, T_gen=[6])
    lay = layout(
        B=1,
        patch_shapes=[[(2, 2)]],
        image_token_offsets=[[0]],
        gen_mask=gen_mask,
    )
    sal = tensor(B=1, T_gen=6, T_img=4)

    base = DummyOutput(dummy=torch.randn(1, 5))
    grid = SaliencyGrid(sal, lay)

    out = SaliencyOutput(base_output=base, saliency=grid)
    assert torch.equal(out.dummy, base.dummy)
    assert out.saliency is grid
