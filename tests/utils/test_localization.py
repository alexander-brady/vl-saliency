...
from vl_saliency.utils.localization import (
    _elbow_chord,
    _identify_localization_heads,
    _spatial_entropy,
    extract_localization_heads,
)
...
def test_basic_selection_and_ordering_by_entropy():
    ...
    out = _identify_localization_heads(t, chord_thresholding=False, min_keep=1, max_keep=5)
    assert out == [(2, 0), (2, 1)]
...
def test_localization_head_selection_respects_chord_thresholding():
    ...
    out = _identify_localization_heads(t, chord_thresholding=True, min_keep=1, max_keep=5)
...
def test_respects_max_keep():
    ...
    out = _identify_localization_heads(t, chord_thresholding=False, min_keep=1, max_keep=1)
    assert out == [(2, 1)]
...
def test_fallback_min_keep_uses_top_attn_sum():
    ...
    out = _identify_localization_heads(t, chord_thresholding=False, min_keep=2, max_keep=None)
...
def test_raises_when_no_heads_and_min_keep_zero():
    ...
    with pytest.raises(ValueError, match="No heads were selected"):
        _identify_localization_heads(t, chord_thresholding=False, min_keep=0, max_keep=None)


# ------------------------------ Extract ------------------------------- #


def test_extract_localization_heads():
    t = torch.zeros(3, 2, 2, 2)
    t[2, 0, 0, 0] = 0.3
    ...
    out = _identify_localization_heads(t, **params)
    assert out == [(2, 0)]

    tensor = t[[2], [0]]  # shape: [1, H, W]
    transformed = extract_localization_heads(t, **params)  # shape: [1, 1, H, W]
    assert torch.equal(transformed, tensor)
