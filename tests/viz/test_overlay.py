...
@pytest.fixture
def smap():
    # Create a simple saliency map with values from 0 to 1
    return torch.linspace(0, 1, steps=16 * 16).reshape(16, 16)
...
def test_overlay_returns_figure_without_image(smap):
    fig = plot(smap, image=None, show_colorbar=False)
...
def test_overlay_with_image_and_colorbar(smap, dummy_image):
    fig = plot(smap, image=dummy_image, show_colorbar=True)
...
def test_overlay_uses_existing_ax(smap, dummy_image):
    fig, ax = plt.subplots()
    fig2 = plot(smap, image=dummy_image, ax=ax, show_colorbar=False, title="custom")
...
def test_overlay_custom_kwargs_passed_to_imshow(smap):
    fig = plot(smap, show_colorbar=False, cmap="viridis", alpha=0.7)
