...
def render_token_ids(
    ...
    Args:
        ...
        processor (ProcessorMixin | PreTrainedTokenizerBase): The processor used to process input.
    ...
    for i, (token, tid) in enumerate(zip(tokens, token_ids, strict=False)):
        ...
        # Render token span
        span = (
            f'<div class="token" style="{COMMON} {style}" title="{html.escape(title)}">{fmt}</div>'
        )
        buffer.append(span)
